...
若依管理系统功能测试与接口自动化测试项目。

- `my_skills/`：存放功能测试用例设计 Skill、测试规范及 Excel 测试用例。
- `ruoyi-api-smoke/`：存放基于 pytest 的若依接口自动化冒烟测试代码。

# my_skills

用于辅助设计若依项目的功能测试点、手工测试用例、冒烟用例、回归用例和 Bug 报告。

```text
my_skills/
└─ cases-skill/
	├─ SKILL.md                         # Skill 使用规则
	├─ cases/                           # 测试用例及相关 Excel 文件
	└─ references/                      # 模板、测试设计和回归规则
		├─ test_case_templates.md        # 测试用例模板
		├─ test_to_case_rules.md         # 测试点转测试用例规则
		├─ test_design_methods.md        # 等价类、边界值、场景法等
		├─ test_case_traceability.md     # 测试点与用例追溯
		├─ test_priority.md              # 优先级和回归分类
		├─ smoke_case.md                 # 冒烟用例规则
		├─ regression_testing.md         # 回归测试规则
		└─ bug_report_templates.md       # Bug 报告模板
```

## Skill 使用方式

在对话中使用 `cases-skill`，并说明目标文件和任务，例如：

```text
使用 cases-skill，检查并完善这个测试点 Excel，输出新的文件，不覆盖原文件。
使用 cases-skill，根据测试点 Excel 生成手工测试用例。
使用 cases-skill，根据已执行用例生成 Bug 报告。
使用 cases-skill，从功能用例中提取功能冒烟和接口冒烟用例。
```

## 使用前提

- 测试文档生成：准备测试点、需求或已有 Excel 文件。
- 接口用例执行：准备 Python 3.10+、若依服务、Redis，并完成 `ruoyi-api-smoke/requirements.txt` 依赖安装。
- 具体地址、账号、Excel 和 Redis 配置见下方“环境准备”和 `ruoyi-api-smoke/config/config.py`。

## 与接口自动化项目的关系

- `cases-skill` 负责测试设计、用例整理、覆盖分析和文档生成。
- `ruoyi-api-smoke` 负责使用 pytest 执行接口自动化测试。
- 接口冒烟用例应优先参考 `ruoyi-api-smoke/cases/`、`config/config.py`、`conftest.py` 和本 README。
- 不确定的接口地址、请求参数、业务码和响应字段不得凭经验填写，应根据接口文档、源码、抓包或现有代码确认。

## 生成内容的基本要求

- 测试点只描述“测什么”；详细步骤和数据放在测试用例中。
- 测试用例应包含前置条件、测试数据、步骤、预期结果和优先级。
- 实际结果、是否通过和备注由测试执行后填写。
- 未确认的业务规则标记为“待确认”，不自行假设。
- 新文件默认另存，避免覆盖原始 Excel。
